...
export function SupabaseSignUpForm({ redirectUrl }: Props) {
  ...
  const handleEmailSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Silently notify backend to dispatch welcome email via Resend
      fetch('/api/email/login-notify', {
        method: 'POST',
      ...
      // If user session is returned immediately (confirm email disabled)
      if (data.session) {
    ...
    <div className="w-full max-w-md bg-slate-900/90 border border-slate-800/90 rounded-3xl p-6 sm:p-8 backdrop-blur-2xl shadow-2xl relative overflow-hidden">
      ...
      <div className="text-center mb-6">
        ...
        <p className="text-xs sm:text-sm text-slate-400 mt-1">
          Join thousands of developers competing in real-time quizzes.
        </p>
      </div>
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl bg-slate-800/90 hover:bg-slate-750 border border-slate-700/80 hover:border-slate-600 text-white text-sm font-semibold transition-all shadow-sm group cursor-pointer disabled:opacity-50"
      >
        {isGoogleLoading ? (
      ...
      <div className="relative my-6 text-center">
        ...
        <span className="relative px-3 text-xs font-bold text-slate-500 bg-slate-900 uppercase tracking-wider">
          Or with email
        </span>
      </div>
